...
const WeaknessFinder = () => {
    // Add a new state for opening statistics
    ...
    const processGames = async (games) => {
        ...
        const stats = {};
            ...
            const processSingleGame = (game, index) => {
                try {
                    chess.loadPgn(game);
                    ...
                    // Update the statistics for each opening
                    setOpeningStats(prevStats => {
            ...
            // Process each game asynchronously to allow state updates after each game
            for (let i = 0; i < games.length; i++) {
                await new Promise(resolve => setTimeout(() => {
                    processSingleGame(games[i], i);
                    resolve();
                }, 0)); // Use a short timeout to allow React to update
            }
            
            setProcessing(false); // Stop processing once all games are done
        ...
        games.forEach((game, index) => {
            try {
                ...
                let opening = detectOpening(moves);
                const result = chess.header()["Result"];
                ...
                fullValList.push(valList);
                 // Initialize stats for this opening if not already present
                 if (!stats[opening]) {
                    stats[opening] = {
                        numGames: 0,
                        wins: 0,
...
// Update the opening stats in real-time
setOpeningStats((prevStats) => {
    const updatedStats = { ...prevStats };
    if (!updatedStats[opening]) {
        updatedStats[opening] = {
...
// SUBSET CHECK
    ...
    <p style={{ fontStyle: 'italic', fontSize: 20, marginLeft: 'auto' }}>Punish your opponent's recurrent shortcomings!</p>
</div>
                    {!showAdvancedFilter ? (
